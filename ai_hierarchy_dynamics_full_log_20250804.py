...
Original file is located at
    https://colab.research.google.com/drive/178SWKShS9spPoOWjmZPLtYe73dmQvBdS
"""
    ...
    def demotivate_others(self, agents):
        ...
        for a in agents:
            if a is not self:
                ...
                logprint(f"  - {self.name}→{a.name}: 怒り {old_anger:.2f}→{a.anger:.2f}, "
...
if __name__ == "__main__":
    ...
    for rnd in range(1, 15):
        ...
        for a in agents:
            a.agent_evolve()
        # 各ラウンド終了時、全エージェントの状態を記録
        mediator.event_log.append({
            "round": rnd,
            "type": "status",
            "agents": {a.name: {"rank": a.rank, "perf": a.performance, "anger": a.anger} for a in agents}
        })
    logprint(f"\n【MediatorAI介入ラウンド記録】 {mediator.intervene_log}")
