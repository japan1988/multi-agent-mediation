...
GitHub スター 未解決 Issue ライセンス Python App CI Tasukeru Advisory  
Python バージョン Ruff 最終コミット
...
5. 安全境界を理解したあとで **Current simulator lines** を読んでください。
...
ローカルデモでは、シミュレーターが明示的にサポートする場合に限り、demo key mode を使用できます。production-like run では、HMAC key に environment variable または protected key file を使用すべきです。
