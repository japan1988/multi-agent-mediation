...
Tasukeru Analysis は、マルチエージェントやエージェント型 workflow の複雑化を確認・統治するための、助言型レビュー・監査レイヤーとして位置づけられます。これらの workflow が複雑になるほど、挙動の把握、説明、監査が難しくなるためです。

主に次のような観点を確認します。

- 検出結果が適切なレビュー優先度に分類されているか
- 処理ログと生成された出力が一致しているか
- 不確実またはリスクのあるケースが HITL へ戻されているか
- 公開 PR コメントに過剰な詳細が含まれていないか
- workflow が自動修正、commit、push、merge を行わない advisory-only の範囲に留まっているか

この workflow は、詳細な攻撃手順を公開するものでも、自律的に執行するシステムでもありません。詳細な検出内容は、人間が確認するために workflow artifacts / logs に保存されます。
