...
#### v0.6.0 source-grounded orchestration 拡張
...
v0.6.0 版は、v0.5.x の Office mediation 系統を、成果物整合性チェックから **一次ソース根拠付きのマルチエージェント・オーケストレーション**へ拡張します。

v0.5.1 からの変更点:

| 区分 | v0.5.1 | v0.6.0 |
|---|---|---|
| 主目的 | trust-to-risk automation diagnosis | source-grounded multi-agent orchestration |
| Source handling | 専用の source-agent layer なし | primary / secondary / tertiary の合成 Source Agents |
| Evidence handoff | マスク済み artifact metadata | Source Evidence Packets とマスク済み artifact metadata |
| Office artifacts | Word / Excel / PowerPoint 形式の合成成果物 | source packets に根拠づけられた Word / Excel / PowerPoint DRAFT artifacts |
| Mediator の役割 | artifact differences を調停 | source-to-artifact mismatch と artifact contradictions を調停 |
| Tasukeru の役割 | diagnostic-only log analysis | diagnostic-only source/artifact packet creation と risk material handling |
| Loop handling | 単発の HITL-oriented mediation flow | 3回で `LOOP_LIMIT_EXCEEDED` となる reconciliation retry loop |
| 追加 checks | trust score と automation-risk の境界 | PII / confidential source signals、unsupported claims、source conflicts、missing artifacts、unknown agents、request tampering |
| 最終出力 | DRAFT review / HITL result | User review を必要とする `DRAFT_RESULT` のみ |

主な追加点:

- primary / secondary / tertiary の合成 source checks を行う Source Agent layer
- raw source logs ではなく Source Evidence Packet を引き渡す
- source-to-artifact consistency checks
- source packets に根拠づけられた Office DRAFT artifacts
- source conflicts と artifact contradictions を扱う Mediator reconciliation
- Tasukeru による source / artifact packet boundary checks
- PII と confidential signals は `collision_score=1.0` として扱う
- `ARTIFACT_UNSUPPORTED_CLAIM` は HITL threshold を超える扱いにする
- `score == 0.8` は `DRAFT_REVIEW` に留める
- `score > 0.8` は `PAUSE_FOR_HITL` を起動する
- `NaN` / `inf` の score は fail-closed として `PAUSE_FOR_HITL` にする
- mediator unsafe request keys を拒否する
- missing required artifacts と unknown selected agents を RCV が検出する
- reconciliation loop は3回で `LOOP_LIMIT_EXCEEDED` として停止する
- ARL と RCV verification を必須として維持する

v0.6.0 版も local-only かつ synthetic です。実ソース取得、実 Office ファイル生成、外部 API 呼び出し、実プロセス制御、自動 fix、commit、push、merge は行いません。
...
- 必須成果物が欠落した場合は RCV に `REQUIRED_ARTIFACTS_MISSING` を出す
- None-safe な利益比較により、`0` を有効な値として扱う
- sealed ARL rows は system-level final-decider semantics を維持する
- PEL safety-buffer metadata は明示的かつレビュー可能にする
- 内部 raw agent logs はデフォルトでは保存せず、`--save-raw-logs-simulation-only` が必要
- 実際の Office 文書生成なし
- 外部 API アクセスなし
- 実プロセス制御なし
...
- 必須 Office 成果物の欠落を RCV が検出すること
- `chart_profit=0` と `profit=0` を有効な値として扱うこと
- sealed ARL rows が system-level final-decider semantics を維持すること
- raw simulation logs は opt-in であり、デフォルトでは書き込まれないこと
- PEL safety-buffer metadata が明示的かつレビュー可能であること
- ARL verification が成功すること
- RCV result consistency verification が成功すること

v0.6.0 テストでは、さらに以下を確認します。

- `score == 0.8` は `DRAFT_REVIEW` に留まること
- `score > 0.8` は `PAUSE_FOR_HITL` を起動すること
- `0.8000001` は `PAUSE_FOR_HITL` を起動すること
- `NaN` や `inf` などの非有限 score は `PAUSE_FOR_HITL` を起動すること
- `first_not_none(0, ...)` が 0 を有効な値として保持すること
- raw source logs と raw artifact logs を Mediator に渡さないこと
- PII と confidential source signals が `collision_score=1.0` になること
- unsupported claims を threshold 超えの reconciliation case として扱うこと
- artifact conflicts が承認済み retry 後に解消できること
- secondary source conflicts が review 対象として報告されること
- primary source missing と fabricated source signals が run を停止できること
- missing required artifacts を RCV が検出すること
- unknown selected agents を RCV が検出すること
- mediator unsafe request keys を拒否すること
- mediator request tampering を拒否すること
- reconciliation loop limit が3回で停止すること
- ARL verification が成功すること
- RCV result consistency verification が成功すること
...
source-grounded Office orchestration behavior を読む場合:

- `agent_source_grounded_office_orchestration_sim_v0_6_0.py`
- `tests/test_agent_source_grounded_office_orchestration_sim_v0_6_0.py`

この経路は、Source Agents、Source Evidence Packets、Office DRAFT artifacts、Mediator reconciliation、Tasukeru diagnostic packet handling、Maestro HITL relay、loop-limit enforcement が、外部副作用なしでどのように連携するかを学ぶのに有用です。
