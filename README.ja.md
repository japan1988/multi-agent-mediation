...
Tasukeru Analysis、通称「助ける君」は、防御的なリポジトリレビューのための
助言型ワークフローです。

次のような静的チェックおよびリポジトリ固有の確認を実行します。
...
このワークフローは、保守性、安全性レビュー、開発者の使いやすさを高めることを
目的としています。すべての advisory finding を自動的にブロッキング失敗として
扱うものではありません。

### HITL 判断支援

Tasukeru Analysis は、検出結果をレビュー段階に分類します。

これにより、保守者は「修正すべき項目」と「想定されるノイズ」を分離できます。

現在のレビュー分類は次のとおりです。

- `HITL_REQUIRED`: マージ前に人間による確認が推奨される
- `FIX_RECOMMENDED`: 具体的な修正が適切である可能性が高い
- `REVIEW_RECOMMENDED`: 文脈を確認してから判断する
- `NOISE_CANDIDATE`: テスト、デモ、研究用シミュレーションでは許容される可能性が高い
- `INFO_ONLY`: 情報提供のみ

この分類は、初期状態では助言目的です。  
保守者が、検出結果を修正するか、記録するか、抑制するか、許容するかを判断するための
補助情報として扱ってください。
...
- `B603` subprocess 実行:
  - 通常は `HITL_REQUIRED`
  - subprocess 実行は外部副作用を生む可能性があるため、人間による確認が必要

- `B404` subprocess import:
  - 通常は `REVIEW_RECOMMENDED`
  - `subprocess` を import するだけでは外部副作用ではない

- `B101` テスト内の assert 使用:
  - 通常は `NOISE_CANDIDATE`
  - pytest のテストでは `assert` の使用が一般的であるため

- `B101` 実行時コード内の assert 使用:
  - 通常は `FIX_RECOMMENDED`
  - Python の最適化実行時に `assert` が除去される可能性があるため

- `B311` シミュレーション内の疑似乱数使用:
  - 通常は `REVIEW_RECOMMENDED`
  - 秘密情報、トークン、認証、認可に使っていない決定論的シミュレーション用途では
    許容される可能性があるため

- `pip-audit` による依存関係の脆弱性:
  - 通常は `FIX_RECOMMENDED`

### 出力される成果物

Tasukeru Analysis は、次のレビュー成果物を生成する場合があります。
...
`tasukeru_hitl_review.md` は、人間が読みやすいレビューガイダンスを提供します。
主に次の情報を含みます。
...
`tasukeru_hitl_review.json` は、同じ判断支援データを機械可読形式で提供します。

### 助言型の挙動

Tasukeru Analysis は、初期状態では助言型です。

次の判断を補助することを目的としています。
...
このワークフローは、人間のレビューを置き換えるものではありません。
より安全なリポジトリ保守のための、トリアージおよび文書化支援として扱ってください。
