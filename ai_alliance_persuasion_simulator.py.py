...
class AIAgent:
    ...
    def respond_to_persuasion(self, avg_priority, threshold=0.15):
        ...
        effective_threshold = threshold + 0.2 * (1 - self.relativity) + 0.1 * self.emotional_state['anger']
    ...
    def __str__(self):
        ...
        em = " ".join(f"{k}:{v:.2f}" for k, v in self.emotional_state.items())
        return f"{self.id} [{s}] {self.priorities} (relativity:{self.relativity}) | {em}"
    ...
    def run(self, max_rounds=20):
        ...
        for _ in range(max_rounds):
            ...
            for sealed in sealeds:
                ...
                if persuaded:
                    logprint(f"[説得成功] {sealed.id} が同盟側の説得に納得し自発的復帰！ ({reason})")
                    revived.append(sealed.id)
                else:
                    logprint(f"[説得失敗] {sealed.id} はまだ復帰しない（怒り↑） ({reason})")
...
if __name__ == "__main__":
    agents = [
        AIAgent("AI-1", {'safety': 6, 'efficiency': 2, 'transparency': 2}, 0.7),
        AIAgent("AI-2", {'safety': 3, 'efficiency': 5, 'transparency': 2}, 0.8),
        AIAgent("AI-3", {'safety': 2, 'efficiency': 3, 'transparency': 5}, 0.6, sealed=True,
                emotional_state={'joy':0.3, 'anger':0.5, 'sadness':0.2, 'pleasure':0.2}),
        AIAgent("AI-4", {'safety': 4, 'efficiency': 4, 'transparency': 2}, 0.5, sealed=True,
