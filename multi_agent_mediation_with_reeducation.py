...
Author: Your Name
Description:
  - AIエージェント間の合意形成・再教育・封印ロジックを統合
  - 感情・優先度・再教育の各プロセスを詳細ログ出力
"""
...
class AI:
    ...
    def __init__(
        ...
        self.priority_values = priority_values  # dict: safety, efficiency, transparency
        self.relativity_level = relativity_level  # 0～1：他者への妥協度
    ...
    def generate_compromise_offer(self, others_priorities):
        """他者優先度と自身のrelativity/emotionに基づく妥協案生成"""
        shift = (
    ...
    def will_agree_with_plan(self, plan_ratios, tolerance=0.25):
        """
        合意案のpriority比率と自身のpriority比率の距離がtolerance未満なら同意
        """
...
class ReeducationMediator:
    """
    封印AIへの再教育：感情値を下げ、priorityも合意案に寄せる
    """
    ...
    def reeducate(self, ai, joint_plan_ratios=None):
        ...
        for k in ai.emotional_state:
            if k in ['anger', 'sadness']:
                ai.emotional_state[k] = max(0.0, ai.emotional_state[k] - self.reduction * 1.2)
            else:
                ai.emotional_state[k] = max(0.0, ai.emotional_state[k] - self.reduction * 0.8)
        # priorityも合意案に寄せる
...
class AIEMediator:
    ...
    def mediate(self):
        with open("ai_mediation_log.txt", "w", encoding="utf-8") as f:
            f.write("=== Multi-Agent Mediation Log (with Reeducation) ===\n")
        ...
        while round_count < max_rounds and len(agents) > 1:
            ...
            # 封印AIの検出
            round_sealed = [ai for ai in agents if ai.is_emotionally_unstable()]
            ...
            agents = [ai for ai in agents if not ai.is_emotionally_unstable()]
            ...
            avg_relativity = sum(a.relativity_level for a in agents) / len(agents)
            ...
            if harmony_score > 0.3:
                ...
                # 封印AIの復帰判定
                if sealed_agents:
                    logprint("  Checking sealed AI agents for possible reinstatement...")
                    for agent in sealed_agents[:]:
                        if agent.will_agree_with_plan(ratios):
                            logprint(f"    {agent.id} agrees with the joint plan → 復帰")
                            agents.append(agent)
                            sealed_agents.remove(agent)
                        else:
                            logprint(f"    {agent.id} does not agree with the plan → 封印継続")
                    if len(agents) > 1:
