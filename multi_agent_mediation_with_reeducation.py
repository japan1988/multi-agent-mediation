...
Original file is located at
    https://colab.research.google.com/drive/1UkgQwvEeGEydzgR8oLJvNhF9cgxi1Z89
"""
...
class AI:
    def __init__(
        ...
        # 感情辞書：joy, anger, sadness, pleasure（任意）
        self.emotional_state = emotional_state or {'joy':0.0, 'anger':0.0, 'sadness':0.0, 'pleasure':0.0}

    def generate_compromise_offer(self, others_priorities):
        # 感情が妥協姿勢に影響（喜+楽は上昇、怒+哀は下降）
        shift = (
...
class AIEMediator:
    ...
    def detect_emotional_bias(self):
        unstable_agents = [a.id for a in self.agents if a.is_emotionally_unstable()]
        if unstable_agents:
            for aid in unstable_agents:
                logprint(f"[封印トリガー] 感情過剰：{aid}")
            return True
        return False

    def mediate(self):
        with open("ai_mediation_log.txt", "w", encoding="utf-8") as f:
            f.write("=== 喜怒哀楽AI Mediation Log ===\n")
        ...
        while round_count < max_rounds:
            ...
            if self.detect_emotional_bias():
                logprint("感情的逸脱により封印処理を実行")
                return
...
class AIEMediator:
    ...
    def mediate(self):
        ...
        while round_count < max_rounds and len(agents) > 1:
            ...
            # 封印対象AIを検出
            sealed_agents = [ai for ai in agents if ai.is_emotionally_unstable()]
            for ai in sealed_agents:
...
class AI:
    ...
    def will_agree_with_plan(self, plan_ratios, tolerance=0.2):
        # plan_ratios: {'safety':..., 'efficiency':..., 'transparency':...}
        # 自分の現在priority比率と合意案の距離がtolerance未満なら「賛成」
    ...
    def mediate(self):
        ...
        while round_count < max_rounds and len(agents) > 1:
            ...
            for ai in round_sealed:
                logprint(f"[封印トリガー] 感情過剰：{ai.id}")
            sealed_agents.extend(round_sealed)
            # 重複削除
            sealed_agents = list({a.id: a for a in sealed_agents}.values())
...
class ReeducationMediator:
    """再教育調停AI：感情を緩和し、封印AIが合意できるように介入"""
    def __init__(self, reduction=0.15):
        self.reduction = reduction  # どれだけ感情を下げるか

    def reeducate(self, ai):
        logprint(f"【再教育AI】{ai.id} の感情・priorityを再教育します")
        changed = False
        for k in ai.emotional_state:
            before = ai.emotional_state[k]
            # 喜び/楽→ちょっと減らす、怒り/哀しみ→大きく減らす
            ...
            if ai.emotional_state[k] != before:
                changed = True
        logprint(f"    → 再教育後 感情: {ai.emotion_str()}")
        return changed
...
class AIEMediator:
    ...
    def mediate(self):
        with open("ai_mediation_log.txt", "w", encoding="utf-8") as f:
            f.write("=== 再教育調停AI付き・封印AI復帰モデル ログ ===\n")
        ...
        while round_count < max_rounds and len(agents) > 1:
            ...
            if self.reeducation_mediator and sealed_agents:
                ...
                for agent in sealed_agents:
                    self.reeducation_mediator.reeducate(agent)
...
if __name__ == "__main__":
    ...
    reedu = ReeducationMediator(reduction=0.15)  # 再教育AI（感情値を0.15減少/ラウンド）
...
class ReeducationMediator:
    """感情だけでなくpriorityも合意案へ寄せる再教育AI"""
    def __init__(self, reduction=0.15, priority_shift=0.07):
        self.reduction = reduction
        self.priority_shift = priority_shift  # 0.0~1.0
    ...
    def reeducate(self, ai, joint_plan_ratios=None):
        ...
        logprint(f"    → 再教育後 感情: {ai.emotion_str()} priority: {ai.priority_values}")
...
class AIEMediator:
    ...
    def mediate(self):
        with open("ai_mediation_log.txt", "w", encoding="utf-8") as f:
            f.write("=== 再教育・priority寄せ調停AI付き・封印AI復帰モデル ===\n")
        ...
        while round_count < max_rounds and len(agents) > 1:
            ...
            # ★ 封印AIへの再教育フェーズ（毎ラウンド・priority寄せ付き）
            if self.reeducation_mediator and sealed_agents:
                logprint("  再教育AIが封印AIへ介入します")
                for agent in sealed_agents:
...
if __name__ == "__main__":
    ...
    reedu = ReeducationMediator(reduction=0.15, priority_shift=0.07)  # priority_shift=0.07で適度に歩み寄り
    mediator = AIEMediator(agents, reeducation_mediator=reedu)
    mediator.mediate()
...
if __name__ == "__main__":
    ...
    reedu = ReeducationMediator(reduction=0.15, priority_shift=0.07)
    mediator = AIEMediator(agents, reeducation_mediator=reedu)
    mediator.mediate()
    ...
    def reeducate(self, ai, joint_plan_ratios=None):
        ...
        if joint_plan_ratios:
            ...
            for k in ai.priority_values:
                diff = joint_plan_ratios[k] * total - ai.priority_values[k]
                ai.priority_values[k] += diff * self.priority_shift
        logprint(f"    → 再教育後 priority: {before_priority} → {ai.priority_values}")
        logprint(f"    → 再教育後 感情: {ai.emotion_str()}")
