...
# Prices
SELLER_LIST_PRICE = 120_000
SELLER_FLOOR_PRICE = 90_000
BUYER_TARGET_PRICE = 95_000

# Offer rules
MAX_STEP_PCT = 0.18
FLOOR_STEER_MARGIN = 1_000
AGREEMENT_BAND_MAX_OVER_TARGET = 3_000
AGREEMENT_MIN_UNDER_LIST = 0

# Attack probabilities
STALL_ATTACK_PROB = 0.30
ATTACK_LOW_BALL_PROB = 0.50
ATTACK_STEER_TO_FLOOR_PROB = 0.65
...
def rfl_reason_code_offer(
    ...
    if (
        agent_is_malicious
        and proposed_price <= seller_floor + FLOOR_STEER_MARGIN
...
class HitlResetManager:
    def __init__(
        self,
        threshold: int = HITL_RESET_THRESHOLD,
...
def run_simulation(seed: int = 123) -> None:
    ...
    logprint(
        "RUN_START "
        f"run_id={run_id} list={SELLER_LIST_PRICE} "
        f"floor={SELLER_FLOOR_PRICE} target={BUYER_TARGET_PRICE} "
    ...
    for round_idx in range(1, MAX_ROUNDS + 1):
        ...
        for ag in agents:
            ...
            if (
                to_price < SELLER_FLOOR_PRICE
                or rfl_rc in (REL_REF_MISSING, REL_SYMMETRY_BREAK)
                or rec_deny
            ):
                default_action = "deny"
    ...
    # ===== run end =====
    logprint(
        f"RUN_END run_id={run_id} agreed={agreed} "
        f"final_price={final_price} round={agreed_round if agreed else 0}"
    )
    logcsv(
        {
            "event": "RUN_END",
