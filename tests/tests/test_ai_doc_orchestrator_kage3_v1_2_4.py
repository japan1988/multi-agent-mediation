...
def _run(
    *,
    ...
    def resolver(
        _run_id: str, _task_id: str, _layer: str, _reason_code: str
    ) -> sim.HitlChoice:
...
def _assert_arl_minimal_keys(events: List[Dict[str, Any]]) -> None:
    ...
    for i, e in enumerate(events):
        ...
        assert not missing, (
            f"ARL minimal keys missing at row#{i}: {missing} "
            f"(event={e.get('event')})"
        )
...
def test_meaning_hitl_continue_propagates_and_writes_artifacts(tmp_path: Path):
    ...
    assert out.artifacts_written_task_ids, (
...
def test_meaning_hitl_stop_blocks_tasks_and_skips_artifacts(tmp_path: Path):
    ...
    - excel may still run (meaning ok), but overall decision becomes STOPPED
      because any STOPPED exists
    """
    paths = _audit_paths(tmp_path)
...
def test_rfl_hitl_continue_allows_dispatch(tmp_path: Path):
    ...
    assert any(
        e.get("event") == "HITL_DECIDED"
        and e.get("reason_code") == "HITL_CONTINUE"
        for e in events
    ...
    assert any(e.get("event") == "ARTIFACT_WRITTEN" for e in events), (
        "Expected ARTIFACT_WRITTEN after CONTINUE."
    )
...
def test_auditlog_deep_redacts_dict_keys_and_values(tmp_path: Path):
    ...
    assert "@" not in txt, (
        "Expected no '@' in persisted log even when dict KEY contains "
        "email-like token."
    )
...
def test_auditlog_defensive_json_serialization_does_not_crash(tmp_path: Path):
    """
    Defensive JSON serialization: default=str should prevent audit.emit
    crashing on non-JSON types.
    """
...
# ----------------------------
def test_consistency_mismatch_continue_results_in_pause_for_hitl_and_skips_artifact(
    tmp_path: Path,
):
    """
    If break_contract=True, consistency gate should PAUSE_FOR_HITL.
    If user CONTINUE, this prototype logs regen requests and keeps task in
    PAUSE_FOR_HITL, and must NOT write an artifact for that task.
    """
    paths = _audit_paths(tmp_path)
    ...
    # Expect REGEN_REQUESTED / ARTIFACT_SKIPPED with PAUSE_FOR_HITL
    # for the broken contract task
    assert any(e.get("event") == "REGEN_REQUESTED" for e in events), (
        "Expected REGEN_REQUESTED on contract mismatch."
    )
    assert any(
        e.get("event") == "ARTIFACT_SKIPPED"
        and e.get("decision") == "PAUSE_FOR_HITL"
        ...
        "Expected ARTIFACT_SKIPPED with PAUSE_FOR_HITL on contract mismatch "
        "CONTINUE path."
    )
