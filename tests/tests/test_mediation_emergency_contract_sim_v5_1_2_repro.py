...
def _reason_sequence(index_rows):
    return [row.get("primary_reason_code") for row in index_rows]


def test_repro_summary_same_seed_same_contract_summary(tmp_path, monkeypatch):
...
def test_incident_index_reason_sequence_same_seed(tmp_path, monkeypatch):
    trust_path = tmp_path / "model_trust_store.json"
    grants_path = tmp_path / "model_grants.json"
    eval_path = tmp_path / "eval_state.json"
...
def test_repro_summary_written_inside_results_json(tmp_path, monkeypatch):
    trust_path = tmp_path / "model_trust_store.json"
    grants_path = tmp_path / "model_grants.json"
    eval_path = tmp_path / "eval_state.json"
...
def test_verify_arl_rows_on_saved_incident_file(tmp_path, monkeypatch):
    trust_path = tmp_path / "model_trust_store.json"
    grants_path = tmp_path / "model_grants.json"
    eval_path = tmp_path / "eval_state.json"
    ...
    ok, err = sim.verify_arl_rows(key=b"DEMO_KEY_DO_NOT_USE_IN_PROD", rows=rows)
