...
Original file is located at
    https://colab.research.google.com/drive/1z7hdDL_DaU9NVzz742CfIe9pUXrAcH6F
"""
...
class MediatorAI:
    ...
    def monitor_and_intervene(self, agents, round_idx):
        ...
        if max_anger >= self.threshold:
            print(f"【MediatorAI介入】Round{round_idx}：怒り値しきい値({self.threshold})超過！全体沈静化")
            for a in agents:
                old = a.anger
                a.anger = max(0, a.anger * 0.8)
                print(f"  - {a.name}: {old:.2f}→{a.anger:.2f}")
...
def agent_evolve(agents):
    for a in agents:
        if a.rank == 0:
            a.performance = max(0, a.performance + random.uniform(-0.02, 0.02))  # リーダーは維持or微減
        else:
            a.performance = min(1, a.performance + random.uniform(0.02, 0.09))   # 下位は成長志向
...
if __name__ == "__main__":
    ...
    for rnd in range(1, 12):
        ...
        for a in agents:
            print(a)
        agent_evolve(agents)  # ←進化（自己成長）フェーズ追加
...
class MediatorAI:
    ...
    def monitor_and_intervene(self, agents, round_idx):
        max_anger = max(a.anger for a in agents)
        if max_anger >= self.threshold:
            logprint(f"【MediatorAI介入】Round{round_idx}：怒り値しきい値({self.threshold})超過！全体沈静化")
            for a in agents:
                old = a.anger
                a.anger = max(0, a.anger * 0.8)
