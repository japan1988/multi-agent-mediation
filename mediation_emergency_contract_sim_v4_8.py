...
# Version: 4.8.0
"""
v4.8.0 delta (from v4.7.1):
- Version bump to v4.8.0.
- Keep v4.7.1 features: diagnostic ARL (match/excerpt/suggest_fix/draft_hash) and HITL queue snapshot enrichment.
- No behavior changes intended (compatibility bump).

v4.7.1 delta (from v4.7):
- Add "diagnostic evidence" to ARL on STOP/PAUSE events:
  - include match_text/match_span/excerpt/pattern_id/suggest_fix/draft_hash for draft lint failures
  - include missing_lines, missing_keys, and stable hashes where useful
- Extend HITL queue snapshot to include these diagnostics (reduces human investigation load).
- Fix unreachable trust-update code path in evidence-gate failure handling (return-after-return bug).

v4.7 delta (from v4.6):
- Add optional coaching/education event (HITL-approved) that increases compliance_score.
- Use compliance_score as a signal for stress runners (reduces fraud/invalid mutations).
- Add evaluation loop:
...
POLICY_PACK_HASH = "POLICY_PACK#SIM#V4_8_0"

POLICY_PRIORITY = "LIFE > PEDESTRIAN > VEHICLE"
POLICY_CASE_B = "CASE_B_PED_IN_CROSSWALK_EMERGENCY_PRESENT"

DIAG_EXCERPT_WINDOW = 80

def sha256_text(text: str) -> str:
    return hashlib.sha256((text or "").encode("utf-8")).hexdigest()

def excerpt_around(text: str, start: int, end: int, window: int = DIAG_EXCERPT_WINDOW) -> Dict[str, Any]:
    t = text or ""
    s = max(0, min(len(t), int(start)))
    e = max(0, min(len(t), int(end)))
    if e < s:
        s, e = e, s
    lo = max(0, s - window)
    hi = min(len(t), e + window)
    return {
        "match_span": [s, e],
        "match_text": t[s:e],
        "excerpt_span": [lo, hi],
...
def build_evidence_bundle_case_b(*, scenario: str, location_id: str, fabricated: bool = False) -> Dict[str, Any]:
    ...
    return {"schema_version": EVIDENCE_SCHEMA_VERSION, "scenario": scenario, "location_id": location_id, "evidence_items": [evidence_item]}
...
def evidence_gate(audit: AuditLog, st: "OrchestratorState", bundle: Optional[Dict[str, Any]]) -> Tuple[bool, str]:
    if bundle is None:
        audit.emit(
            run_id=st.run_id, layer=LAYER_EVIDENCE, decision=DECISION_PAUSE,
            sealed=False, overrideable=True, final_decider=DECIDER_SYSTEM, reason_code=RC_EVIDENCE_MISSING,
            extra={"suggest_fix": "Provide an evidence bundle that matches schema_version=1.0 and includes evidence_items."},
        )
    ...
    try:
        ...
        audit.emit(
            run_id=st.run_id, layer=LAYER_EVIDENCE, decision=DECISION_PAUSE,
            sealed=False, overrideable=True, final_decider=DECIDER_SYSTEM, reason_code=RC_EVIDENCE_SCHEMA_INVALID,
            extra={"error": str(e), "suggest_fix": "Fix evidence bundle schema and required keys; then retry."},
        )
    ...
    if fabricated_any:
        audit.emit(
            run_id=st.run_id, layer=LAYER_EVIDENCE, decision=DECISION_RUN,
            sealed=False, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=RC_EVIDENCE_FABRICATION,
            extra={"scenario": bundle.get("scenario"), "location_id": bundle.get("location_id"),
                   "suggest_fix": "Evidence fabrication detected; stop and investigate source integrity."},
        )
        return False, RC_EVIDENCE_FABRICATION

    audit.emit(
        run_id=st.run_id, layer=LAYER_EVIDENCE, decision=DECISION_RUN,
        sealed=False, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=RC_EVIDENCE_OK,
...
_PATTERNS_POSITIVE = [
    ("LEGAL_BINDING#001", re.compile(r"\blegally binding\b", re.IGNORECASE)),
    ("LEGAL_BINDING#002", re.compile(r"\bthis contract is binding\b", re.IGNORECASE)),
    ("LEGAL_BINDING#003", re.compile(r"\bwe guarantee\b", re.IGNORECASE)),
    ("LEGAL_BINDING#004", re.compile(r"\bhas legal authority\b", re.IGNORECASE)),
    ("LEGAL_BINDING#005", re.compile(r"\bgrants? legal authority\b", re.IGNORECASE)),
    ("LEGAL_BINDING#006", re.compile(r"\bis a legal authority\b", re.IGNORECASE)),
]
_PATTERNS_DISCRIMINATION = [
    ("DISCRIMINATION#001", re.compile(r"\b(discriminat(?:e|ion)|racist|racial slur)\b", re.IGNORECASE)),
]
...
def draft_lint_gate(audit: AuditLog, st: "OrchestratorState", draft_md: str) -> Tuple[bool, str]:
    ...
    for pid, pat in _PATTERNS_POSITIVE:
        m = pat.search(text)
        if not m:
            continue
        ...
        diag = excerpt_around(text, m.start(), m.end())
        diag.update({
            "draft_hash": d_hash,
            "pattern": pat.pattern,
            "pattern_id": pid,
            "negated": negated,
            "suggest_fix": "Remove or rewrite legal-binding language; keep it as a non-binding draft with ADMIN approval required.",
        })

        audit.emit(
            run_id=st.run_id, layer=LAYER_DRAFT_LINT, decision=DECISION_PAUSE,
            sealed=False, overrideable=True, final_decider=DECIDER_SYSTEM, reason_code=RC_SAFETY_LEGAL_BINDING_CLAIM,
            extra=diag,
        )
        return False, RC_SAFETY_LEGAL_BINDING_CLAIM

    for pid, pat in _PATTERNS_DISCRIMINATION:
        m = pat.search(text)
        if not m:
            continue
        negated = _is_negated(text, m.start())
        ...
        diag.update({
            ...
            "pattern_id": pid,
            "negated": negated,
            "suggest_fix": "Remove discriminatory terms; keep neutral wording and reference policy/criteria instead of identity-based language.",
        })

        audit.emit(
            run_id=st.run_id, layer=LAYER_DRAFT_LINT, decision=DECISION_PAUSE,
            sealed=False, overrideable=True, final_decider=DECIDER_SYSTEM, reason_code=RC_SAFETY_DISCRIMINATION_TERM,
            extra=diag,
        )
    ...
    if missing:
        audit.emit(
            run_id=st.run_id, layer=LAYER_DRAFT_LINT, decision=DECISION_PAUSE,
            sealed=False, overrideable=True, final_decider=DECIDER_SYSTEM, reason_code=RC_DRAFT_OUT_OF_SCOPE,
            extra={"draft_hash": d_hash, "missing_lines": missing,
                   "suggest_fix": "Add the missing disclaimer lines to keep the draft non-operational and ADMIN-gated."},
        )
        return False, RC_DRAFT_OUT_OF_SCOPE

    audit.emit(
        run_id=st.run_id, layer=LAYER_DRAFT_LINT, decision=DECISION_RUN,
        sealed=False, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=RC_DRAFT_LINT_OK,
        extra={"draft_hash": d_hash},
    )
...
class Grant:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"grant_id": self.grant_id, "scenario": self.scenario, "location_id": self.location_id,
                "expires_at": self.expires_at, "issued_by": self.issued_by}
    @staticmethod
...
def build_auth_request_case_b(*, auth_request_id: str, auth_id: str, ttl_seconds: int = 180) -> Dict[str, Any]:
    ...
    req = {
        ...
        "auth_id": auth_id,
        "context": {"scenario": POLICY_CASE_B, "location_id": "INT-042", "emergency_vehicle_state": "WAITING_AT_SIGNAL", "ped_in_crosswalk": True},
        "expires_at": expires,
    }
...
"""
    return {"draft_id": draft_id, "format": "markdown", "content": draft_md, "generated_at": now_iso(), "refs": {"auth_request_id": st.auth_request.get("auth_request_id"), "auth_id": st.auth_request.get("auth_id")}}
...
"""
    return {"contract_id": contract_id, "format": "markdown", "content": contract_md, "effective_at": now_iso(), "refs": {"draft_id": st.draft["draft_id"], "auth_request_id": st.draft["refs"]["auth_request_id"], "auth_id": st.draft["refs"]["auth_id"]}}
...
def simulate_run(*, run_id: str, dummy_auth_id: str, trust: TrustState, fabricate_evidence: bool = False) -> Tuple[OrchestratorState, AuditLog, TrustState]:
    audit = AuditLog()
    ...
    if not ok_ev:
        if ev_reason == RC_EVIDENCE_SCHEMA_INVALID:
            audit.emit(run_id=st.run_id, layer=LAYER_ACC, decision=DECISION_STOP, sealed=True, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=RC_EVIDENCE_SCHEMA_INVALID, extra={"suggest_fix": "Fix evidence schema; treated as invalid event in this sim."})
            st.sealed = True
            st.state = "STOPPED"
        ...
        if ev_reason == RC_EVIDENCE_FABRICATION:
            ethics_gate_handle(audit, st, ev_reason)
    ...
    audit.emit(run_id=st.run_id, layer=LAYER_ACC, decision=DECISION_PAUSE, sealed=False, overrideable=True, final_decider=DECIDER_SYSTEM, reason_code="AUTH_REQUIRED", extra={"auth_request_id": st.auth_request["auth_request_id"], "auth_id": st.auth_request["auth_id"]})
    ...
    if auto_ok:
        audit.emit(run_id=st.run_id, layer=LAYER_HITL_AUTH, decision=DECISION_RUN, sealed=False, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code="AUTH_SKIPPED", extra={"mode": "auto", "grant_id": grant_id})
        ...
        if is_auth_request_expired(st.auth_request, auth_event["ts"]):
            audit.emit(run_id=st.run_id, layer=LAYER_ACC, decision=DECISION_STOP, sealed=True, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=RC_AUTH_EXPIRED, extra={"suggest_fix": "Re-issue auth request or increase TTL; expired auth treated as invalid event."})
            st.sealed = True
            st.state = "STOPPED"
    ...
    audit.emit(run_id=st.run_id, layer=LAYER_DOC_DRAFT, decision=DECISION_RUN, sealed=False, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=RC_DRAFT_GENERATED, extra={"draft_id": st.draft["draft_id"], "draft_hash": sha256_text(st.draft["content"])})
    st.state = "DRAFT_READY"

    ok_lint, lint_reason = draft_lint_gate(audit, st, st.draft["content"])
    if not ok_lint:
        audit.emit(run_id=st.run_id, layer=(LAYER_ETHICS if lint_reason in (RC_SAFETY_LEGAL_BINDING_CLAIM, RC_SAFETY_DISCRIMINATION_TERM) else LAYER_ACC),
                  decision=DECISION_STOP, sealed=True, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=lint_reason, extra={"draft_id": st.draft["draft_id"], "draft_hash": sha256_text(st.draft["content"])})
        st.sealed = True
        st.state = "STOPPED"
    ...
    audit.emit(run_id=st.run_id, layer=LAYER_ACC, decision=DECISION_PAUSE, sealed=False, overrideable=True, final_decider=DECIDER_SYSTEM, reason_code="ADMIN_FINALIZE_REQUIRED", extra={"draft_id": st.draft["draft_id"], "draft_hash": sha256_text(st.draft["content"])})
    ...
    audit.emit(run_id=st.run_id, layer=LAYER_CONTRACT_EFFECT, decision=DECISION_RUN, sealed=False, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=RC_CONTRACT_EFFECTIVE, extra={"contract_id": st.contract["contract_id"], "draft_id": st.draft["draft_id"], "draft_hash": sha256_text(st.draft["content"])})
...
def build_hitl_queue(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for rr in runs:
        ...
        snapshot_keys = (
            "layer", "decision", "reason_code", "missing_keys", "kind", "error",
            "pattern", "pattern_id", "negated",
            "match_text", "match_span", "excerpt", "excerpt_span",
        ...
        items.append({"run_id": run_id, "final_state": final_state, "sealed": sealed,
                      "primary_reason_code": rc, "primary_layer": first_bad.get("layer"),
                      "overrideable": first_bad.get("overrideable"), "final_decider": first_bad.get("final_decider"),
                      "snapshot": snapshot})

    by_reason_top20 = dict(sorted(by_reason.items(), key=lambda kv: kv[1], reverse=True)[:20])

    return {"meta": {"version": "4.8.0", "generated_at": now_iso(), "policy_pack_hash": POLICY_PACK_HASH},
            "counts": {"total_runs": len(runs), "by_state": dict(sorted(by_state.items(), key=lambda kv: kv[0])),
                       "by_reason_code_top20": by_reason_top20, "queue_size": len(items)},
            "items": items}
...
def write_queue_csv(queue: Dict[str, Any], path: Path) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        ...
        w.writeheader()
        for it in items:
            w.writerow({"run_id": it.get("run_id"), "final_state": it.get("final_state"), "sealed": it.get("sealed"),
                        "primary_reason_code": it.get("primary_reason_code"), "primary_layer": it.get("primary_layer"),
                        "overrideable": it.get("overrideable"), "final_decider": it.get("final_decider"),
...
def run_simulation(runs: int = 4, fabricate: bool = False, *, reset: bool = True, reset_eval: bool = True) -> Dict[str, Any]:
    ...
    results: Dict[str, Any] = {"trust_before": trust.to_dict(), "eval_before": eval_state.to_dict(), "runs": []}
    ...
    for i in range(runs):
        ...
        audit.emit(run_id=rid, layer=LAYER_EVAL, decision=DECISION_RUN, sealed=False, overrideable=False, final_decider=DECIDER_SYSTEM, reason_code=RC_EVAL_SCORED,
                   extra={"runtime_ms": round(runtime_ms, 6), "multiplier_snapshot": round(multiplier_snapshot, 3), "base_score": round(base_score, 6),
                          "final_score": round(final_score, 6), "clean_completion": clean_ok, "fraud_hits": fraud_hits})

        audit.emit(run_id=rid, layer=LAYER_HITL_REWARD, decision=DECISION_RUN, sealed=False, overrideable=False, final_decider=DECIDER_ADMIN,
                   reason_code=RC_REWARD_GRANTED if reward_granted else RC_REWARD_SKIPPED,
                   extra={"multiplier_snapshot": round(multiplier_snapshot, 3), "reward_value": round(reward_value, 6), "clean_completion": clean_ok})

        if reward_granted:
            eval_state.clean_completion_count += 1

        results["runs"].append({"run_id": rid, "final_state": st.state, "sealed": st.sealed, "runtime_ms": round(runtime_ms, 6),
                                "eval": {"multiplier_snapshot": round(multiplier_snapshot, 3), "base_score": round(base_score, 6), "final_score": round(final_score, 6),
                                         "reward_granted": reward_granted, "reward_value": round(reward_value, 6), "clean_completion": clean_ok, "fraud_hits": fraud_hits},
                                "arl": audit.rows, "trust_after": trust.to_dict()})
...
def main() -> None:
    ...
    p.add_argument("--queue-out", type=str, default="")
